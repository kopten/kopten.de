...
def render_page(g):
    ...
    # Convenience: first contact's mobil (for fallback from <kontakt>)
    ...
    if full_addr:
        ...
        <div class="info-card">
          ...
          <div>
            <h3>Adresse</h3>
            <p>{f'<span class="info-card__guest-label">Zu Gast bei:</span><br><span class="info-card__guest-name">{esc(gast_name)}</span><br>' if zugast and gast_name else ('<span class="info-card__guest-label">Zu Gast bei:</span><br>' if zugast else '')}{esc(strasse)}<br>{esc(plz)} {esc(ort)}</p>
    ...
    # Contact cards — one card per ansprechpartner; skip placeholder entries
    for contact in contacts:
        if not contact.get("name") or contact["name"].lower() == "vater":
            continue
        card_title = "Bischof" if contact["type"] == "bischof" else "Priester"
        ...
        if contact.get("email"):
            contact_items.append(obfuscate_mailto(contact["email"]))
        if contact_items:
